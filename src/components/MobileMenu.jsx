    ...
    <div className={`fixed top-0 left-0 w-full z-40 bg-[rgba(10,10,10,0.8)] flex flex-col items-center justify-center
     ...
     ${menuOpen ? 'h-screen opacity-100 pointer-events-auto' : 'h-0 opacity-0 pointer-events-none'}`}>
        <button onClick={() => setMenuOpen(false)} className='absolute top-6 right-6 text-white focus:outline-none' aria-label='Close Menu'>
            ...
            <a href="#contactus" className={`text-2xl font-semibold text-white my-4 transform transition-transform duration-300 ${menuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} onClick={() => setMenuOpen(false)} >Contact us</a>
    </div>
  )
}
